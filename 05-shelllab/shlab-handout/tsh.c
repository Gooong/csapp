...
volatile sig_atomic_t fg_pid = 0;  /* fg pid */
...
void eval(char *cmdline) 
{
    ...
    int pid;
    // should we deep copy argv? The memory will be overwrite if parseline is called later
    // -- no need, the execve will deep copy argv.
...
{  
	...
	//if (job->state!=ST) deletejob(jobs, pid);
...
void print_log(char* message){
    if (verbose) printf("log: %s", message);
}
